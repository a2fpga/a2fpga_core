...
// High-level SPI service wrapper implementation
#include "a2fpga_spi_service.h"
#include <string.h>

static ospi_link_t s_link;
...
esp_err_t a2spi_init_once(spi_host_device_t host, const ospi_pins_t *pins, int clock_hz)
{
    ...
    esp_err_t err = ospi_link_init(&s_link, host, pins, clock_hz);
    if (err == ESP_OK) {
...
esp_err_t a2spi_deinit(void)
{
    ...
    esp_err_t err = ospi_link_cleanup(&s_link);
    s_inited = false;
    return err;
}
...
{
    return s_inited && s_link.octal_mode;
}
