 ...
 * Uthernet II card, a2mega / ESP32-S3 port. See w5100.h for the overview.
 *
 * Port of boards/a2n20v2-Enhanced/src/a2n20_bl616/firmware_host/w5100.c:
 *   fpga_spi_xfer_*  ->  fpga_mem_read/write(A2SPACE_W5100, ...)
 *   fpga_spi_reg_*   ->  fpga_reg_read/write
 *   FPGA_REG_U2_CMD_PENDING -> A2REG_U2_DOORBELL
 * Compound register/buffer sequences are wrapped in fpga_link_lock/unlock so
 * ring-pointer updates stay atomic against other FPGA-link users (menu, disk).
 *
 ...
 * XFER memory SPACE 3, and act:
 *   - MACRAW OPEN  -> start bridging socket 0 to the WiFi uplink
 *   - MACRAW SEND  -> drain the TX ring and transmit the raw frame
 *   - MACRAW RECV  -> recompute Sn_RX_RSR from the Apple II's read pointer
 *   - CLOSE        -> stop bridging
 * Wire frames arrive via w5100_macraw_rx() (called from wifi_bridge_poll), are
 * MAC-filtered, framed with the 2-byte length header, and pushed into the RX ring.
 *
 * The actual TX/MAC-NAT plumbing lives in wifi_bridge.c behind the weak hooks
 * below.
 */
...
static const char *TAG = "w5100";

/* ---- weak bridge hooks (overridden in wifi_bridge.c) ---- */
__attribute__((weak)) void w5100_bridge_tx(const uint8_t *frame, uint32_t len) { (void)frame; (void)len; }
__attribute__((weak)) bool w5100_bridge_uplink_mac(uint8_t mac[6]) { (void)mac; return false; }
__attribute__((weak)) void w5100_bridge_set_uplink_mac(const uint8_t mac[6]) { (void)mac; }
...
static bool    g_uplink_mirrored; /* uplink told the Apple II SHAR (NAT seeded) */
static bool    g_shar_seeded;     /* SHAR preloaded with the uplink MAC */

/* Diagnostics surfaced on the DebugOverlay (scratch regs 0x0C-0x0F) */
...
/* ---- command handlers (callers hold the FPGA link lock) ---- */
...
static void macraw_open(int n)
{
    ...
    fpga_mem_read(A2SPACE_W5100, W5100_SHAR, g_mac, 6);
    ...
    g_uplink_mirrored = false;   /* w5100_sync_mac() will push SHAR -> uplink */
    ESP_LOGI(TAG, "MACRAW open: rx %u tx %u mf %d mac %02x:%02x:%02x:%02x:%02x:%02x",
             s->rx_size, s->tx_size, (int)g_macraw_mf,
...
static void sock_close(int n)
{
    ...
    if (n == 0 && g_macraw_active) {
        ...
        g_uplink_mirrored = false;
        ESP_LOGI(TAG, "MACRAW closed");
    }
}

/* Keep the uplink informed of the Apple II SHAR (MAC-NAT seed) and, when no
 * stack has claimed a MAC yet, preload SHAR with the uplink's MAC so stacks
 * that read their MAC from the card adopt it (which makes the NAT a no-op).
 * Idempotent; polled each w5100_poll so it handles both bring-up orders. */
static void w5100_sync_mac(void)
{
    uint8_t umac[6];
    if (!w5100_bridge_uplink_mac(umac)) {
        /* Uplink not up yet -> reset so we re-apply on (re)connect */
        g_uplink_mirrored = false;
        g_shar_seeded = false;
        return;
    }
...
#else
    if (g_macraw_active) {
        /* Tell the uplink the Apple II's MAC (e.g. IP65's cfg_mac). */
        if (g_mac_valid && !g_uplink_mirrored) {
            w5100_bridge_set_uplink_mac(g_mac);
            g_uplink_mirrored = true;
        }
    } else if (!g_shar_seeded) {
        /* Preload SHAR with the uplink MAC so stacks that read their MAC from
         * the card adopt it (stacks that write their own SHAR are handled
         * above). */
        fpga_mem_write(A2SPACE_W5100, W5100_SHAR, umac, 6);
        g_shar_seeded = true;
    }
#endif
}
...
static void macraw_send(int n)
{
    ...
    if (first >= len) {
        fpga_mem_read(A2SPACE_W5100, s->tx_base + off, g_frame, len);
    } else {
        fpga_mem_read(A2SPACE_W5100, s->tx_base + off, g_frame, first);
        fpga_mem_read(A2SPACE_W5100, s->tx_base, g_frame + first, len - first);
    }
 ...
 *
 * Why this is in the poll loop and not w5100_init: on the BL616 board, SPACE-3
 * writes issued while the Apple II bus and card were still settling did NOT
 * land in BSRAM (verified on HW), so the seed retries until a read-back
 * confirms -- self-healing across the startup window. Kept on the ESP32 port:
 * it is harmless when the first write lands and protects the same race. */
static bool w5100_seed_defaults(void)
{
    ...
    fpga_reg_write(A2REG_U2_DOORBELL, 0x0F);  /* clear stale doorbell */

    /* Confirm RTR (0x0017) read-back == 0x07: the write actually reached BSRAM. */
    bool ok = (w_rd8(0x0017) == 0x07);
    fpga_link_unlock();
    if (ok)
        ESP_LOGI(TAG, "reset defaults seeded");
    return ok;
}
 ...
 * over HDMI). Leaves 0x07 (overlay byte0) alone -- the gamepad path uses it.
 * Overlay layout:
 *   byte1 (0x0C) = status: b0 macraw_active, b1 uplink_mirrored, b2 mac_valid,
...
static void w5100_report(void)
{
    ...
    uint8_t st = (g_macraw_active ? 0x01 : 0) | (g_uplink_mirrored ? 0x02 : 0) |
                 (g_mac_valid ? 0x04 : 0) | (g_macraw_mf ? 0x08 : 0) |
                 (hb ? 0x10 : 0) | (g_defaults_seeded ? 0x80 : 0);

    fpga_reg_write(A2REG_SCRATCH1, st);
    fpga_reg_write(A2REG_SCRATCH2, (uint8_t)g_rx_frames);
    fpga_reg_write(A2REG_SCRATCH3, (uint8_t)g_tx_frames);
    fpga_reg_write(A2REG_SCRATCH4, (uint8_t)g_drop_full);
}

void w5100_poll(void)
{
    /* Self-healing seed: retry the reset-default registers until a read-back
     * confirms they landed (see w5100_seed_defaults). Once confirmed, stop. */
    if (!g_defaults_seeded)
        g_defaults_seeded = w5100_seed_defaults();

    w5100_sync_mac();   /* keep uplink MAC NAT / SHAR seed current (every tick) */

    /* Throttle the DebugOverlay to ~2 Hz at a ~1 kHz poll cadence -- updating
     * it every poll makes the hex unreadable. Command servicing below still
     * runs every tick. */
    static uint16_t rpt;
    if (++rpt >= 500) { rpt = 0; w5100_report(); }

    uint8_t pending = fpga_reg_read(A2REG_U2_DOORBELL) & 0x0F;
    if (!pending) return;
    ...
    fpga_reg_write(A2REG_U2_DOORBELL, pending);
    fpga_link_unlock();
}
...
void w5100_macraw_rx(const uint8_t *frame, uint32_t len)
{
    ...
    if ((uint32_t)used + rec >= s->rx_size) {
        g_drop_full++;
        fpga_link_unlock();
        return;
    }
    ...
    if (first >= total) {
        fpga_mem_write(A2SPACE_W5100, s->rx_base + off, g_frame, total);
    } else {
        fpga_mem_write(A2SPACE_W5100, s->rx_base + off, g_frame, first);
        fpga_mem_write(A2SPACE_W5100, s->rx_base, g_frame + first, total - first);
    }
